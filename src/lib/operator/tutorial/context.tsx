...
export function OperatorDryRunProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { setSession((current) => current ? changeSessionLocale(current, locale) : current); }, [locale]);
  ...
  const exit = useCallback(() => {
    setSession((current) => {
      if (current?.status === "active" && !window.confirm(t("dryRun.exitConfirm"))) return current;
      sessionStorage.removeItem(DRY_RUN_STORAGE_KEY);
      return null;
    });
  ...
  const value = useMemo<DryRunContextValue>(() => ({ active: session?.status === "active", session, start: () => setSession(createDryRunSession(locale)), exit, emit }), [emit, exit, locale, session]);
  ...
  return <DryRunContext.Provider value={value}>
    ...
    {children}
    {session?.status === "active" && step ? <TutorialOverlay session={session} step={step} total={completeShopDaySteps.length} onBack={() => setSession((current) => current ? goBack(current) : current)} onNext={() => setSession((current) => current ? advanceInstruction(current) : current)} onExit={exit} onRestart={() => { if (window.confirm(t("dryRun.restartConfirm"))) setSession((current) => current ? restartSession(current) : current); }} onRecover={() => { setSession((current) => current ? restartSession(current) : current); router.replace("/operator"); }} /> : null}
