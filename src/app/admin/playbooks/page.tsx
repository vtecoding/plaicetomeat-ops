    ...
    <PageFrame>
      <main className="mx-auto max-w-4xl px-4 pb-28 pt-6 sm:px-6 lg:px-8" data-testid="playbooks-page">
        <Masthead
          ...
          eyebrow="How to run the shop"
          title="Operational playbooks"
          subtitle="Short, plain-English how-to guides for each job. The briefing links you straight to the right one when something needs doing."
        />

        <Link
          href="/admin/playbooks/butcher-words"
          className="mt-6 flex items-center gap-4 rounded-2xl border border-[#bfe3cf] bg-[#f2fbf5] p-5 shadow-sm transition hover:-translate-y-0.5 hover:bg-[#eafaf0] hover:shadow-md"
        >
          <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[var(--brand)] text-white">
            <GraduationCap className="h-6 w-6" aria-hidden />
          </span>
          <span className="min-w-0">
            <span className="block text-lg font-semibold text-[var(--brand)]">New to butchery? Start here</span>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {playbooks.map((playbook) => (
            ...
            >
              <BookOpen className="h-6 w-6 text-[var(--brand)]" aria-hidden />
              <p className="mt-3 text-lg font-semibold text-[var(--ink)]">{playbook.title}</p>
