...
Generated: 2026-08-22T20:22:01.308Z
...
- PASS: Do Now is above the fold — all priorities visible without scrolling — zone top=298px bottom=715px (viewport 1000px)
- PASS: at most three primary actions render — Do now = 3: Check Chicken Breast Fillets now | Expired Cert Meats certificate is expired | Verification Pending Foods certificate needs renewal
- PASS: the 'How the shop is doing' status panel is retired — shop-status not present
- PASS: weekly summary never outranks actions (below Do Now, collapsed) — present=false below=true collapsed=true
