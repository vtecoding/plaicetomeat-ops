...
2. **All writes behind RPCs or server actions** following the existing pattern (`SECURITY DEFINER` RPCs with branch/role validation, or service-client actions with `resolveStaffContext`). No client-side Supabase writes.
...
| D-3 | Which products sell by each/box; confirm they start "untracked" | A2 (final catalogue pass only) | All current `each`/`box` products → `untracked` |
...
* RPC `record_order_tender(p_order_id, p_direction, p_method, p_idempotency_key, p_reason default null)` — SECURITY DEFINER; validates staff+branch; derives `amount_pence` server-side from the order's effective subtotal (later: final subtotal after B4 amendments); inserts event + `audit_logs` row; ON CONFLICT (idempotency_key) DO NOTHING RETURNING existing (retry-safe).
* RPC `collect_order_with_tender(p_order_id, p_method, p_idempotency_key, p_note default null)` — one transaction: calls `transition_order_status(p_order_id,'collected',p_note)` then `record_order_tender(..., 'sale', ...)`. **New name, not an overload** (rule 1.4). Ready-state validation stays inside the transition RPC.
...
* New service `src/lib/server/payment-truth.ts`: `getDayPaymentPicture(branchId, dateIso)` → `{ expectedCashPence, expectedCardPence, cashSales, cardSales, refunds, ordersMissingTender[] }`. Expected cash includes the opening float read from today's opening checklist session payload (`float_ready.value`). Orders collected **without** a payment event (legacy rows, or pre-A1 data) are listed as `missingTender`, never guessed.

**Closing checklist (definition bump + migration 2 `v18_close_money_steps`):**
* `cash_counted` step gains server-provided context: the operator page fetches `getDayPaymentPicture` and renders "Expected in till: £X" above the input (prefill NOT the expected value — the operator must count; only show it). Payload saves `{ value, expected_pence }`.
...
**Guards/tests:**
* Unit: expected-cash/card math incl. float, refunds, missing-tender listing; tender idempotency (same key twice → one row); serve retry path (existing runId replay must not double-write tender).
* DB script `scripts/verify-payment-truth.mjs`: collect twice → one event; refund direction respected; variance alert fires above threshold, not below.
* Static `verify:payment-truth` added to the gate list; E2E: counter collect with tender happy path.
* Failure-first checks from the audit: double-tap collect, two staff collecting the same order (second gets "already collected", no second event), tender insert failure → whole RPC rolls back (no collected-without-tender within the new path).
...
* `products.inventory_policy text not null default 'kg_batch' check in ('kg_batch','untracked')`; backfill: `unit_type in ('each','box') → 'untracked'`. No depletion change (kg filter stays); no each-batch tables (explicitly rejected by audit).
...
**Stock surfaces:** inventory + purchasing show a plain "Not stock-tracked" badge on `untracked` products (no fake zero-stock warnings for them; purchasing excludes them from stock-cover maths and says why).
...
### B3 — Refunds + operator mistake flag (PTM-OPS-005) — scope S (depends A1)

* **Server action `refundOrder`** (manager-gated): inputs order, lines (or full), reason, stock outcome per line (`restock` → existing `admin_reverse_order_inventory`; `discard` → waste event via existing RPC; `none`). Writes `record_order_tender(direction='refund', method = original tender's method, amount = computed)`, audit log, and an owner alert when ≥ D-6 value. Idempotent per (order, reason) — mirrors the reversal RPC's key.
...
* **Tests:** refund idempotency; refund > order total rejected; partial line refund math; day receipt shows refund (with C1); E2E refund happy path.

### B4 — Weigh-at-handover amendments (PTM-OPS-006) — scope M (depends A1)

* **Migration `v18_order_amendments`:** table `order_amendments` (`id, branch_id, order_id, order_item_id, kind check in ('weight_adjust','substitute','remove'), old_quantity, new_quantity, old_line_total_pence, new_line_total_pence, substitute_product_id null, actor_id, reason null, idempotency_key unique, created_at`), append-only + audit. View or helper computing `final_subtotal` (base items overlaid with latest amendment per item).
* **RPC `amend_order_item(...)`** — valid only in `ready`/`prepping` states (never after collected — post-collection fixes are B3 refunds); prices recomputed server-side from the catalogue (substitute uses target product's live price; weight_adjust reprices at the snapshot's unit price).
* **Depletion + tender on final quantities:** `collect_order_with_tender` and `deplete_order_inventory` read effective quantities (item overlay). Depletion change is surgical: the RPC's item query joins latest amendment; covered by the existing inventory validation scripts plus new cases.
...
* **Tests:** amendment idempotency; amend-after-collected rejected; depletion consumes final kg; tender amount = final subtotal; E2E catch-weight collect.
...
* No schema change. Flows (serve, stock, waste) call `saveOperatorRun(status:'in_progress', steps: <mode + answers>)` fire-and-forget on each mode transition (debounced; failures ignored silently — drafts are best-effort).
* Entry screens query newest same-day `in_progress` run for (operator, workflow): "Carry on where you left off? / Start fresh" (start-fresh marks the old run `abandoned` — status enum already includes it for checklists; confirm for workflow runs, else write `steps.abandoned=true`).
* Two-device duel: resume shows what's saved; completing either wins by runId idempotency (existing).
* Tests: unit for draft serialisation round-trip; E2E refresh-mid-delivery resume.
...
| --- | --- | --- |
| G-A | Phase A code-done | Physical reconciliation day (§25.3): counted till vs expected within D-8 cash tolerance; terminal Z vs expected card; seeded refund*, catch-weight*, each-item sale, wrong-weight correction all explained from the receipt (*seeded manually pre-B3/B4 via direct records); timed Gul serve rehearsal (kg + each), zero abandoned sales |
| G-B1 | B1 lands | Seeded critical alert received on Dad's phone < 5 min; digest received on a real morning |
| G-B | Phase B code-done | Staged owner-away trial stages 1–2 (§25.4); Dad clears a seeded tray unaided; refund + amendment traced end-to-end on a live day |
...
4. **GitHub Actions secrets are still empty** (pre-existing blocker): B1's digest workflow and the existing backup workflow share this owner task. B1 ships with the in-app + `skipped`-dispatch fallback so it degrades honestly if secrets lag.
